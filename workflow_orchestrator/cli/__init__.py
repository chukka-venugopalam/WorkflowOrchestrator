...
def _get_kernel() -> Any:
    """Get the bootstrapped Orchestrator kernel instance."""
...
def agents() -> None:
    ...
    for agent in detected:
        ...
        agent_type = getattr(agent, "agent_type", "cli")
...
def environment() -> None:
    ...
    env_obj = environment_detector.detect()
    env = env_obj.to_dict() if hasattr(env_obj, "to_dict") else getattr(env_obj, "__dict__", {})
    ...
    # Workspace
    console.print("\n[bold]Workspace[/]")
    ws_obj = workspace_detector.detect()
    workspace = ws_obj.to_dict() if hasattr(ws_obj, "to_dict") else getattr(ws_obj, "__dict__", {})
    ...
    # Dependencies
    console.print("\n[bold]Dependencies[/]")
    deps_obj = dependency_detector.detect(Path.cwd())
    if deps_obj:
        ...
        if deps_obj.languages:
            dep_table.add_row("Languages", ", ".join(deps_obj.languages))
        if deps_obj.frameworks:
            dep_table.add_row("Frameworks", ", ".join(deps_obj.frameworks))
        if deps_obj.databases:
            dep_table.add_row("Databases", ", ".join(deps_obj.databases))
