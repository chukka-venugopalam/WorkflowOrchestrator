"""DeepSeek provider adapter — communicates with DeepSeek's API.

Supports:
- DeepSeek-Coder and DeepSeek-V3 reasoning models
...
class DeepSeekProvider(BaseProvider):
    ...
    def __init__(
        ...
        self._base_url = (base_url or os.environ.get("DEEPSEEK_BASE_URL", "https://api.deepseek.com/v1")).rstrip("/")
    ...
    def manifest(self) -> ProviderManifest:
        return ProviderManifest(
            ...
            name="DeepSeek AI",
            version="1.0.0",
            description="DeepSeek AI reasoning and code generation models (DeepSeek-V3, DeepSeek-R1, DeepSeek-Coder)",
            capabilities=[
                Capability(id="reasoning.analysis", description="DeepSeek reasoning and analysis"),
                ...
                Capability(id="codegen.math", description="Mathematical and algorithmic reasoning"),
            ],
    ...
    async def _initialize_impl(self) -> None:
        if self._simulation_mode:
            logger.info("DEEPSEEK_API_KEY not set. Running DeepSeek provider in SIMULATION_MODE.")
        else:
            logger.info("DeepSeek provider initialized with endpoint '%s'", self._base_url)
    ...
    async def _health_impl(self) -> ProviderHealth:
        if self._simulation_mode:
            return ProviderHealth(
                ...
                message="Running in SIMULATION_MODE (DEEPSEEK_API_KEY not configured)",
            )
        try:
    ...
    async def discover_models(self) -> List[str]:
        """Dynamically query DeepSeek models endpoint."""
        if self._simulation_mode:
            return ["deepseek-coder", "deepseek-chat", "deepseek-reasoner"]
        try:
            req = urllib.request.Request(
                f"{self._base_url}/models",
                headers={"Authorization": f"Bearer {self._api_key}"},
            ...
            logger.warning("Failed to dynamically discover DeepSeek models: %s", exc)
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as resp:
                ...
                return ExecutionResult(
                    ...
                    metadata={"provider": "deepseek", "model": self._model, "simulation_mode": False},
                )
        except Exception as exc:
            logger.error("DeepSeek API execution error: %s", exc)
            return ExecutionResult(
                task_id=request.task_id,
                success=False,
                output="",
                error=str(exc),
                metadata={"provider": "deepseek", "simulation_mode": False},
            )
    ...
    def _simulate_execution(self, request: ExecutionRequest) -> ExecutionResult:
        return ExecutionResult(
            task_id=request.task_id,
            success=True,
            output=f"[DeepSeek SIMULATION_MODE] Completed goal: '{request.goal}' using model '{self._model}'.",
            token_usage={"total": 140},
            metadata={"provider": "deepseek", "simulation_mode": True},
        )
